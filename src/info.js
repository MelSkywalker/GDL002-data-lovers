...
const lookForLocalStorage = () => {
  if (localStorage.getItem("name")) {
    displayInfo(
      POKEMON.pokemon.find( pokemonName => pokemonName.name === localStorage.getItem("name")));
...
const validatePokemon = () => {
  const wantedPokemon = dataLovers.findPokemon(
...
const fail = () => {
  alert(
    "Tu búsqueda no coincide con ningún Pokémon, pero aquí tienes un Pokémon al azar :)"
  );
...
const displayInfo = pokemonObject => {
  ...
  document.getElementById("pokePreEvolution").innerHTML = findEvolution(
    pokemonObject,
    "prev_evolution"
  );
  document.getElementById("pokeEvolution").innerHTML = findEvolution(
    pokemonObject,
    "next_evolution"
  );
  ...
  document.getElementById("pokeDescription").innerHTML =
    pokemonObject.description;
  document.getElementById("divWeaknessesImg").src = findWeaknesses(
    pokemonObject
  );
  document.getElementById("frequencySpawn").innerHTML =
    pokemonObject.avg_spawns;
...
const displayMaxCP = pokemonObject => {
  ...
  for (let i = 0; i < pokemonObject.multipliers.length; i++) {
    if (i != pokemonObject.multipliers.length - 1) {
      ...
      const cpValue = dataLovers.calculateMaxCP(
        pokemonObject.multipliers[i],
        cp
      );
...
document
  .getElementById("infoPageMenu")
  .addEventListener("click", () => displayScreen("info1"));
document
  .getElementById("listPageMenu")
  .addEventListener("click", () => displayScreen("list1"));

document
  .getElementById("searchButton")
  .addEventListener("click", displayPokemon);
...
document
  ...
  .addEventListener("click", () => displayMaxCP(globalPokemon));
